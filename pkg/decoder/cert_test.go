...
func TestDecodePemCert(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "decode failed",
			args: args{
				data: []byte(strings.Replace(pemCertA, "=", "234", -1)),
			},
			wantErr: true,
		},
	}
...
func TestDecodePemPrivateKey(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "decode failed",
			args: args{
				data: []byte(strings.Replace(pemKeyA, "=", "233", -1)),
			},
			wantErr: true,
		},
	}
...
func TestDecodePemCerts(t *testing.T) {
	type args struct {
		data []byte
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			certs, err := DecodePemCerts(tt.args.data)
			if tt.wantErr {
				assert.Error(t, err)
				assert.Nil(t, certs)
			} else {
